...
const populateEventsInner = async (eventType) => {
  ...
  await Moralis.start({
    serverUrl: MORALIS_EVENTS.serverUrl,
    appId: MORALIS_EVENTS.appId
  });
  const events = Moralis.Object.extend(
    eventType == EVENT_TYPE.STAKED ? "StakedEvents" : "UnstakedEvents"
  );
  ...
  console.log(results[0], " from morallis");
  for await (let doc of results) {
    ...
    if (!record) {
      ...
      event.owner_address = doc.attributes.owner;
...
const updateStakingRecords = async () => {
  ...
  let allUnprocessedEvents = await Event.find({
    isProcessed: false,
    hasException: false
    ...
    if (record.eventType == EVENT_TYPE.STAKED) {
      try {
        for await (let tokenId of record.tokenIds) {
          let stakingRecord = await Staking.findOne({
            asset: tokenId,
            endDate: null
          });
          if (!stakingRecord) {
            //check if staking doc already exists
            let staking = new Staking();
            staking.asset = tokenId;
            staking.startDate = record.block_timestamp;
            staking.address = record.owner_address;
      ...
      try {
        let stakingRecord = await Staking.findOne({
          asset: record.tokenIds[0],
          endDate: null
        });
        if (stakingRecord) {
          stakingRecord.address = record.owner_address;
...
cron.schedule("*/15 * * * *", async () => {
  ...
  mongoose
    .connect("mongodb://localhost:27017/LegendaryVault", {
