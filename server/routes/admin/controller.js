const { massageResponses,rename_IdToId,convertRawIDToMongoDBId } = require("../../utilities/utils");
...
const getList = async (model,params = {}) => {
    ...
    try {
        let filteredDocuments = await model.find()
            .limit(_end)
            .skip(_start)
            .sort({[_sort]:_order ? _order.toLowerCase() : undefined}); //.lean();

     //   filteredDocuments = filteredDocuments.map(rename_IdToId);
//         for (var o of filteredDocuments) {
//             for (var key in o) {
//                 if (typeof o[key] == 'object' && key != '_id') {
//                     for (var innerKey in o[key]) {
//                         o[key + '__' + innerKey] = o[key][innerKey];
//                     }
//                 }
//             }                      
// //                        x["__" + a] = x.uuuu[a] }
//         }
        ...
        return massageResponses(200,{
            ...
            total: await model.find({}).count() //(filteredDocuments && filteredDocuments.length) || 0
        });
